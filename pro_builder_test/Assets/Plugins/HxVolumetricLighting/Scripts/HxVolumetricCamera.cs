...
public class HxVolumetricCamera : MonoBehaviour
{
    ...
    RenderTexture FullBlurRT;
    RenderTargetIdentifier FullBlurRTID;

    RenderTexture downScaledBlurRT;
    RenderTargetIdentifier downScaledBlurRTID;

    RenderTexture FullBlurRT2;
    RenderTargetIdentifier FullBlurRT2ID;

    RenderTargetIdentifier[] VolumetricUpsampledBlurTextures = new RenderTargetIdentifier[2];
    RenderTexture[] VolumetricDensityTextures = new RenderTexture[8];
    int[] VolumetricDensityPID = new int[4] { 0, 0, 0, 0 };
    int[] VolumetricTransparencyPID = new int[4] { 0, 0, 0, 0 };
    RenderTexture[] VolumetricTransparencyTextures = new RenderTexture[8];

    public RenderTargetIdentifier[][] VolumetricDensity = new RenderTargetIdentifier[][] {
    ...
    public RenderTargetIdentifier[][] VolumetricTransparency = new RenderTargetIdentifier[][] {
        new RenderTargetIdentifier[2] { new RenderTargetIdentifier(), new RenderTargetIdentifier()},
        new RenderTargetIdentifier[3] { new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier() },
        new RenderTargetIdentifier[4] { new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier() },
        new RenderTargetIdentifier[5] { new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier() },
    ...
    public RenderTargetIdentifier[][] VolumetricTransparencyI = new RenderTargetIdentifier[][] {
        new RenderTargetIdentifier[1] { new RenderTargetIdentifier()},
        new RenderTargetIdentifier[2] { new RenderTargetIdentifier(), new RenderTargetIdentifier() },
        new RenderTargetIdentifier[3] { new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier() },
        new RenderTargetIdentifier[4] { new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier(), new RenderTargetIdentifier() },
    ...
    public RenderTexture[] ScaledDepthTexture = new RenderTexture[4] { null, null, null, null };
    ...
    public RenderTargetIdentifier[] ScaledDepthTextureRTID = new RenderTargetIdentifier[4];
    [System.NonSerialized]
   ...
   // bool warned = false;
    ...
    void CreateTempTextures()
    {
        ...
        if (resolution != Resolution.full) //TODO remove 16bit depth on this texture, might not need it
        {     
            if (FullBlurRT != null && (FullBlurRT.width != GetCamPixelWidth() || FullBlurRT.height != GetCamPixelHeight()))
            {
                FullBlurRT.Release();
                FullBlurRT = null;
            }
            ...
            if (FullBlurRT == null)
            {
                FullBlurRT = new RenderTexture(GetCamPixelWidth(), GetCamPixelHeight(), 16, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
        ...
        if (VolumetricTexture != null && (VolumetricTexture.width != w || VolumetricTexture.height != h))
        {
             VolumetricTexture.Release();
        ...
        {
            VolumetricTexture = new RenderTexture(w, h, 16, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
        ...
        if (ScaledDepthTexture[(int)resolution] != null && (ScaledDepthTexture[(int)resolution].width != w || ScaledDepthTexture[(int)resolution].height != h))
        {
            ScaledDepthTexture[(int)resolution].Release();
            ScaledDepthTexture[(int)resolution] = null;
        }
        ...
        if (ScaledDepthTexture[(int)resolution] == null)
        {
            ...
            ScaledDepthTexture[(int)resolution] = new RenderTexture(w, h, 24, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear); //Need 4 channels for the upsampling.
        ...
        if (TransparencySupport)
        {
            ...
            for (int i = 0; i < (int)compatibleTBuffer() + 1; i++)
            {
                ...
                if (VolumetricTransparencyTextures[i] != null && (VolumetricTransparencyTextures[i].width != w || VolumetricTransparencyTextures[i].height != h))
                {
                    VolumetricTransparencyTextures[i].Release();
                ...
                {
                    VolumetricTransparencyTextures[i] = new RenderTexture(w, h, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear); //this might actually be fine as an 888 texture?
        ...
        {
            if (downScaledBlurRT.width != w || downScaledBlurRT.height != h)
            {
                downScaledBlurRT.Release();
            ...
            if (downScaledBlurRT == null && ((blurCount > 0 || ((BlurTransparency > 0 || (MapToLDR == true)) && TransparencySupport)) && resolution != Resolution.full)) // || Mycamera.hdr == false
        {
            downScaledBlurRT = new RenderTexture(w, h, 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
        ...
        {
            if (FullBlurRT2.width != GetCamPixelWidth() || FullBlurRT2.height != GetCamPixelHeight())
            {
                FullBlurRT2.Release();
                 FullBlurRT2 = null;
            }
            ...
            if (FullBlurRT2 == null && ((resolution != Resolution.full && UpSampledblurCount > 0) || (resolution == Resolution.full && (blurCount > 0 || ((BlurTransparency > 0 || (MapToLDR == true)) && TransparencySupport) || TemporalSampling)) || (MapToLDR))) // || Mycamera.hdr == false
        {
            ...
            FullBlurRT2 = new RenderTexture(GetCamPixelWidth(), GetCamPixelHeight(), 0, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
        ...
        if (ParticleDensitySupport)
        {
            for (int i = 0; i < (int)compatibleDBuffer() + 1; i++)
            {
                ...
                {
                    if (VolumetricDensityTextures[i].width != w || VolumetricDensityTextures[i].height != h)
                    {
                        VolumetricDensityTextures[i].Release();
                        VolumetricDensityTextures[i] = null;
                    }
                }
                    ...
                    if (VolumetricDensityTextures[i] == null)
                {
                    ...
                    VolumetricDensityTextures[i] = new RenderTexture(w, h, (i == 0 ? 16 : 0), RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);//need to create a depth for no reason
    ...
    void CreateSetupBuffer()
    {
        ...
        if (!SetupBufferAdded)
        {
            ...
            if (TransparencySupport)
            {
                ...
                BufferSetup.SetRenderTarget(VolumetricTransparencyI[(int)compatibleTBuffer()], HxVolumetricCamera.Active.ScaledDepthTextureRTID[(int)HxVolumetricCamera.Active.resolution]);
                BufferSetup.ClearRenderTarget(false, true, new Color32(0, 0, 0, 0));
            }

            BufferSetup.SetRenderTarget(ScaledDepthTexture[(int)resolution]);
    ...
    void BuildBuffer()
    {
        ...
        if (dirty) //incase resolution was changed.
        {
            ...
            BufferRender.SetRenderTarget(VolumetricTextureRTID, HxVolumetricCamera.Active.ScaledDepthTextureRTID[(int)HxVolumetricCamera.Active.resolution]);
    ...
    public void ReleaseTempTextures()
    {
        ...
        if (VolumetricTexture != null) { VolumetricTexture.Release(); VolumetricTexture = null; }
        if (FullBlurRT != null) { FullBlurRT.Release(); FullBlurRT = null; }

        for (int i = 0; i < VolumetricTransparencyTextures.Length; i++)
        {
            if (VolumetricTransparencyTextures[i] != null) { VolumetricTransparencyTextures[i].Release(); VolumetricTransparencyTextures[i] = null; }
        }


        for (int i = 0; i < VolumetricDensityTextures.Length; i++)
        {
            if (VolumetricDensityTextures[i] != null) { VolumetricDensityTextures[i].Release(); VolumetricDensityTextures[i] = null; }
        }

        if (downScaledBlurRT != null)
        {
            downScaledBlurRT.Release(); downScaledBlurRT = null;
        }

        if (FullBlurRT2 != null)
        {
            FullBlurRT2.Release(); FullBlurRT2 = null;
        }
        //if (resolution != Resolution.full)
        //{
        if (ScaledDepthTexture[0] != null) { ScaledDepthTexture[0].Release(); ScaledDepthTexture[0] = null; }
        if (ScaledDepthTexture[1] != null) { ScaledDepthTexture[1].Release(); ScaledDepthTexture[1] = null; }
        if (ScaledDepthTexture[2] != null) { ScaledDepthTexture[2].Release(); ScaledDepthTexture[2] = null; }
        if (ScaledDepthTexture[3] != null) { ScaledDepthTexture[3].Release(); ScaledDepthTexture[3] = null; }
    ...
    void CreateLightbuffers()
    {
        ...
        {
            BufferRenderLights.SetRenderTarget(VolumetricTransparency[(int)HxVolumetricCamera.Active.compatibleTBuffer()], HxVolumetricCamera.Active.ScaledDepthTextureRTID[(int)HxVolumetricCamera.Active.resolution]);
        }
        else
        {
            BufferRenderLights.SetRenderTarget(HxVolumetricCamera.VolumetricTextureRTID, HxVolumetricCamera.Active.ScaledDepthTextureRTID[(int)HxVolumetricCamera.Active.resolution]);
        }
