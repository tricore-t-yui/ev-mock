...
public class HxVolumetricLight : MonoBehaviour
{
    ...
    bool ShaderModel4()
    {
       ///Note: this function is not needed as none of the shaders use shader model 4, thats why it returns false.
    ...
    void BuildDirectionalBuffer(CommandBuffer CameraBuffer)
    {
        ...
        bool RenderShadows = LightShadow() != LightShadows.None && Shadows && QualitySettings.shadows != ShadowQuality.Disable;
        ...
        if (dirty)
        {
           ...
           // if (RenderShadows) { Graphics.DrawMesh(HxVolumetricCamera.BoxMesh, Matrix4x4.TRS(HxVolumetricCamera.Active.transform.position, Quaternion.identity, new Vector3(HxVolumetricCamera.Active.MaxDirectionalRayDistance, HxVolumetricCamera.Active.MaxDirectionalRayDistance, HxVolumetricCamera.Active.MaxDirectionalRayDistance) * 2), HxVolumetricCamera.ShadowMaterial, 0, HxVolumetricCamera.ActiveCamera, 0, null, ShadowCastingMode.ShadowsOnly); }
            ...
            if (RenderShadows) //dont have to switch if rendering none shadow casting lights
            {
                ...
                {
                    CameraBuffer.SetRenderTarget(HxVolumetricCamera.Active.VolumetricTransparency[(int)HxVolumetricCamera.Active.compatibleTBuffer()], HxVolumetricCamera.Active.ScaledDepthTextureRTID[(int)HxVolumetricCamera.Active.resolution]);
                }
                else
                {
                    CameraBuffer.SetRenderTarget(HxVolumetricCamera.VolumetricTextureRTID, HxVolumetricCamera.Active.ScaledDepthTextureRTID[(int)HxVolumetricCamera.Active.resolution]);
                }
            }

            CameraBuffer.SetGlobalMatrix(HxVolumetricLight.VolumetricMVPPID, HxVolumetricCamera.Active.BlitMatrixMVP);
            ...
            CameraBuffer.DrawMesh(HxVolumetricCamera.QuadMesh, HxVolumetricCamera.Active.BlitMatrix, HxVolumetricCamera.GetDirectionalMaterial(mid), 0, DirectionalPass(CameraBuffer), propertyBlock);
    ...
    void BuildPointBuffer(CommandBuffer cameraBuffer)
    {
        ...
        if (distanceLerp > 0)
        {
            ...
            if (dirty)
            {
                ...
                int pass = (distance <= (LightRange() + LightRange() * 0.09f) / 2 + HxVolumetricCamera.ActiveCamera.nearClipPlane * 2 ? 0 : 1); //near or far
                ...
                if (LightCookie() != null)
                {
                    ...
                    cameraBuffer.DrawMesh((pass == 0 ? HxVolumetricCamera.QuadMesh : HxVolumetricCamera.SphereMesh), LightMatrix, HxVolumetricCamera.GetPointMaterial(mid), 0, pass, propertyBlock);
                }
                else
                {
