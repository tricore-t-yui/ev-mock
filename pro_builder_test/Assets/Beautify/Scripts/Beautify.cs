...
namespace BeautifyEffect {
    ...
    public class Beautify : MonoBehaviour {
        ...
        [SerializeField, Range(1, 10)]
        int
            _anamorphicUltraResolution = 10;

        public int anamorphicUltraResolution {
            get { return _anamorphicUltraResolution; }
            set {
        ...
        void OnPreRender() {
            ...
            if (_downscale > 1 && rtDescBase.width > 1) {
                ...
                rtPixDesc.width = Mathf.RoundToInt(Mathf.Max(1, currentCamera.pixelWidth / _downscale));
        ...
        void RenderRightEyeDepth() {
            ...
            desc.depthBufferBits = 24;
            desc.colorFormat = RenderTextureFormat.Depth;
            bloomSourceDepthTextureRightEye = RenderTexture.GetTemporary(desc); // RenderTexture.GetTemporary(size, height, 24, RenderTextureFormat.Depth);
            desc.depthBufferBits = 0;
            desc.colorFormat = rtFormat;
            bloomSourceTextureRightEye = RenderTexture.GetTemporary(desc); // RenderTexture.GetTemporary(size, height, 0, rtFormat);
                ...
                if (_anamorphicFlares) {
                    ...
                    if (_quality == BEAUTIFY_QUALITY.BestPerformance) {
                        ...
                        if (_anamorphicFlaresUltra) {
                            sizeAF = (int)Mathf.Lerp(512, source.height, _anamorphicUltraResolution / 10);
                        } else {
                            sizeAF = 512;
                        }
                    }
